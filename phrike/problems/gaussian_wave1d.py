...
class GaussianWave1DProblem(BaseProblem):
    ...
    def create_initial_conditions(self, grid: Grid1D):
        ...
        wave_type = ic_config.get("wave_type", "traveling")  # "traveling" or "stationary"
    ...
    def get_analytical_solution(self, grid: Grid1D, t: float) -> Optional[Dict[str, np.ndarray]]:
        ...
        if wave_type == "stationary":
            ...
            gaussian = amplitude * np.exp(-((x_translated - x0) ** 2) / (2.0 * sigma ** 2))
    ...
    def create_visualization(self, solver, t: float, U):
        ...
        if os.path.exists(frames_dir):
            ...
            if analytical:
                ax[0, 0].plot(x, analytical['rho'], 'r--', linewidth=1, alpha=0.7, label='Analytical')
            ax[0, 0].set_xlabel('x')
            ...
            if analytical:
                ax[0, 1].plot(x, analytical['u'], 'r--', linewidth=1, alpha=0.7, label='Analytical')
            ax[0, 1].set_xlabel('x')
            ...
            if analytical:
                ax[1, 0].plot(x, analytical['p'], 'r--', linewidth=1, alpha=0.7, label='Analytical')
            ax[1, 0].set_xlabel('x')
            ...
            if analytical:
                ...
                ax[1, 1].text(0.5, 0.5, 'No analytical solution', ha='center', va='center', transform=ax[1, 1].transAxes)
                ax[1, 1].set_title('Error vs Analytical')
    ...
    def _create_wave_packet_analysis_plots(self, solver) -> None:
        ...
        if analytical is not None:
            ...
            axes[0, 0].plot(x, analytical["rho"], 'r--', linewidth=2, label='Analytical')
        ...
        if "mass" in history and "momentum" in history and "energy" in history:
            ...
            axes[0, 1].semilogy(times, mass_error, label="Mass", marker='o', markersize=3)
            axes[0, 1].semilogy(times, momentum_error, label="Momentum", marker='s', markersize=3)
            axes[0, 1].semilogy(times, energy_error, label="Energy", marker='^', markersize=3)
        ...
        if len(times) > 1:
            ...
            for i, (idx, color) in enumerate(zip(time_indices, colors)):
                if "density_history" in history:
                    ...
                    axes[0, 2].plot(x, rho_t, color=color, linewidth=1.5, 
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(self.outdir, "wave_packet_analysis.png"), dpi=150, bbox_inches='tight')
        plt.close()
