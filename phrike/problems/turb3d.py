...
class Turb3DProblem(BaseProblem):
    ...
    def create_visualization(self, solver, t: float, U) -> None:
        ...
        if projection_type == "column_density":
            ...
            projection_data = self._create_column_density_projection(rho, solver.grid, video_config)
            ...
            projection_data = self._create_slice_projection(rho, solver.grid, video_config)
        ...
        try:
            im = ax.imshow(projection_data, origin='lower', extent=extent, 
                          aspect='equal', cmap='cmapkk9', norm=norm)
        except ValueError:
        ...
        fig.savefig(os.path.join(frames_dir, f"frame_{t:08.3f}.png"), dpi=frame_dpi, bbox_inches='tight')
        plt.close(fig)
