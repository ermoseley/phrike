...
class Acoustic1DProblem(BaseProblem):
    ...
    def _create_test_analysis_plots(self, solver) -> None:
        ...
        if "mass" in history and "momentum" in history and "energy" in history:
            ...
            axes[0, 0].semilogy(times, mass_error, label="Mass", marker='o', markersize=3)
            axes[0, 0].semilogy(times, momentum_error, label="Momentum", marker='s', markersize=3)
            axes[0, 0].semilogy(times, energy_error, label="Energy", marker='^', markersize=3)
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(self.outdir, "test_analysis.png"), dpi=150, bbox_inches='tight')
        plt.close()
