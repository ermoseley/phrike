    ...
    if getattr(grid, "_legendre_basis", None) is not None:
        try:
            ...
            if wy is not None and wx is not None:
                ...
                # Torch-aware branch to avoid implicit NumPy conversions on device tensors
                is_torch = False
                try:
                    import torch  # type: ignore
                    is_torch = isinstance(U, torch.Tensor)
                ...
                if is_torch:
                    ...
                    rho_c = np.maximum(_ensure_numpy(rho), 1e-12)
                    p_c = np.maximum(_ensure_numpy(p), 1e-12)
                ...
                # Bottom boundary (v=0 wall)
                rho_b = rho[j0, :]
                ux_b = ux[j0, :]
                uy_b = (uy[j0, :] * 0.0)
                p_b = p[j0, :]
                Eb = p_b / (gamma - 1.0) + 0.5 * rho_b * (ux_b * ux_b)
                if is_torch:
                    U_b = torch.stack([rho_b, rho_b * ux_b, rho_b * uy_b, Eb], dim=0)
                else:
                    U_b = np.stack([rho_b, rho_b * ux_b, rho_b * uy_b, Eb], axis=0)

                U_in_bot = U[:, j0, :]
                momy_in = U_in_bot[2]
                if is_torch:
                    Fy_in = torch.empty_like(U_in_bot)
                    Fy_in[0] = momy_in
                    ...
                    Fstar_bot = 0.5 * (Fy_in + Fy_bc) - 0.5 * alpha_y_bot[None, :] * (U_in_bot - U_b)
                rhs[:, j0, :] += -(1.0 / float(wy[0])) * (Fstar_bot - Fy_in)
                ...
                uy_t = uy[jN, :]
                p_t = p[jN, :]
                Et = p_t / (gamma - 1.0) + 0.5 * rho_t * (ux_t * ux_t + uy_t * uy_t)
                if is_torch:
                    U_t = torch.stack([rho_t, rho_t * ux_t, rho_t * uy_t, Et], dim=0)
                    momy_in_t = U_in_top[2]
                    Fy_in_t = torch.empty_like(U_in_top)
                    Fy_in_t[0] = momy_in_t
                    ...
                    Fstar_top = 0.5 * (Fy_in_t + Fy_bc_t) - 0.5 * alpha_y_top[None, :] * (U_in_top - U_t)
                rhs[:, jN, :] += +(1.0 / float(wy[-1])) * (Fstar_top - Fy_in_t)
                ...
                uy_l = uy[:, i0]
                p_l = p[:, i0]
                El = p_l / (gamma - 1.0) + 0.5 * rho_l * (ux_l * ux_l + uy_l * uy_l)
                if is_torch:
                    U_l = torch.stack([rho_l, rho_l * ux_l, rho_l * uy_l, El], dim=0)
                    momx_in_l = U_in_left[1]
                    Fx_in_l = torch.empty_like(U_in_left)
                    Fx_in_l[0] = momx_in_l
                    ...
                    Fstar_left = 0.5 * (Fx_in_l + Fx_bc_l) - 0.5 * alpha_x_left[None, :] * (U_in_left - U_l)
                rhs[:, :, i0] += -(1.0 / float(wx[0])) * (Fstar_left - Fx_in_l)
                ...
                uy_r = uy[:, iN]
                p_r = p[:, iN]
                Er = p_r / (gamma - 1.0) + 0.5 * rho_r * (ux_r * ux_r + uy_r * uy_r)
                if is_torch:
                    U_r = torch.stack([rho_r, rho_r * ux_r, rho_r * uy_r, Er], dim=0)
                    momx_in_r = U_in_right[1]
                    Fx_in_r = torch.empty_like(U_in_right)
                    Fx_in_r[0] = momx_in_r
                    ...
                    Fstar_right = 0.5 * (Fx_in_r + Fx_bc_r) - 0.5 * alpha_x_right[None, :] * (U_in_right - U_r)
                rhs[:, :, iN] += +(1.0 / float(wx[-1])) * (Fstar_right - Fx_in_r)
        except Exception:
            pass
