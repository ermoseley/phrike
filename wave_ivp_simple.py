#!/usr/bin/env python3
"""
Simple 1D wave equation as IVP with proper tau method.
This is a minimal test to understand how tau method works for IVPs.
"""
...
def main():
    """Run a simple wave equation IVP with tau method."""
    ...
    logger.info(f"Setting up simple wave equation IVP with {Nx} Chebyshev points")
    ...
    u_x = dx(u) + lift(tau_1)
    u_xx = dx(u_x) + lift(tau_2)

    # Problem setup with tau terms
    problem = d3.IVP([u, v, tau_1, tau_2], namespace=locals())
    
    # Wave equation in first-order form: dt(u) = v, dt(v) = c^2 * u_xx
    problem.add_equation("dt(u) = v")
    problem.add_equation("dt(v) = c*c*u_xx")
    
    # Boundary conditions - fixed ends (only for u, not v)
    ...
    fig.suptitle(f'Simple Wave Equation IVP (N={Nx}) at t={solver.sim_time:.3f}', fontsize=14)
    ...
    plt.tight_layout()
    plt.savefig('wave_ivp_simple.png', dpi=200, bbox_inches='tight')
    plt.close()

    logger.info('Plot saved as wave_ivp_simple.png')
