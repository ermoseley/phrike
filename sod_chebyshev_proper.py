...
Based on the tau method documentation and waves on a string example.
"""
...
def main():
    ...
    # Create tau terms for boundary conditions (only for velocity)
    ...
    # First-order reduction for second derivatives (only for velocity)
    ...
    problem.add_equation("dt(u) = -u*u_x - dx(p)/rho + nu*u_xx")
    ...
    plt.tight_layout()
    plt.savefig('sod_chebyshev_proper_final.png', dpi=200, bbox_inches='tight')
    plt.close(fig)
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig('sod_chebyshev_proper_density.png', dpi=200, bbox_inches='tight')
    plt.close()

    logger.info('Plots saved as sod_chebyshev_proper_final.png and sod_chebyshev_proper_density.png')
