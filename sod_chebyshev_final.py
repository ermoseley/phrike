#!/usr/bin/env python3
"""
Sod shock tube simulation using Dedalus with proper tau method implementation.
Based on the tau method documentation and working examples.
"""
...
def main():
    """Run the Sod shock tube simulation with proper tau method."""
    
    # Parameters
    Lx = 1.0  # Domain length
    Nx = 128  # Number of Chebyshev modes
    gamma = 1.4  # Ratio of specific heats
    dealias = 3/2  # Dealiasing factor
    stop_sim_time = 0.1  # Simulation end time
    timestep = 1e-6  # Time step
    ...
    # Substitutions for derivatives with tau terms (first-order reduction)
    ...
    plt.tight_layout()
    plt.savefig('sod_chebyshev_final.png', dpi=200, bbox_inches='tight')
    plt.close(fig)
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig('sod_chebyshev_final_density.png', dpi=200, bbox_inches='tight')
    plt.close()

    logger.info('Plots saved as sod_chebyshev_final.png and sod_chebyshev_final_density.png')
