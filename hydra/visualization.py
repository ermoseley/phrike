...
def plot_fields(grid, U, equations, title: str = "", outpath: Optional[str] = None) -> None:
    rho, u, p, _ = equations.primitive(U)
    E = U[2]

    fig, axs = plt.subplots(2, 2, figsize=(10, 7), constrained_layout=True)
    axs[0, 0].plot(grid.x, rho)
    axs[0, 0].set_title("Density")
    axs[0, 1].plot(grid.x, u)
    axs[0, 1].set_title("Velocity")
    axs[1, 0].plot(grid.x, p)
    axs[1, 0].set_title("Pressure")
    axs[1, 1].plot(grid.x, E)
...
def plot_conserved_time_series(history: Dict[str, List[float]], outpath: Optional[str] = None) -> None:
